...
class SetupTopLevelPhase : CompilerPhase<SetupTopLevelPhase>() {
    ...
    override fun run(context: CompilerContext) {
        for (source in context.astCache.getAllSources()) {
            ...
            val pass = SetupTopLevelPass()
            val walker = SimpleWalker()
            pass.doPostPass(source.file, walker)
        }
    }

    companion object : PhaseDescriptor<SetupTopLevelPhase>("Setup top level")
}